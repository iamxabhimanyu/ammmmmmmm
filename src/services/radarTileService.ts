...
export async function reverseGeocodeLocation(lat: number, lon: number): Promise<LocationInfo> {
  // 1. Check if near any major known Indian city (< 15km)
  for (const city of MAJOR_INDIAN_CITIES) {
    const dist = getCoordinatesDistanceKm(lat, lon, city.lat, city.lon);
    if (dist < 15) {
      return {
        ...city,
  ...
  try {
    ...
    if (res.ok) {
      ...
      return {
        id: `map-${lat.toFixed(3)}-${lon.toFixed(3)}`,
        name: cityName,
        state: stateName,
  ...
  // 3. Fallback to nearest city name with coordinates
  ...
  return {
    id: `map-loc-${lat.toFixed(3)}-${lon.toFixed(3)}`,
    name: minDistance < 50 ? `${closest.name} Outskirts` : `Point (${lat.toFixed(2)}°N, ${lon.toFixed(2)}°E)`,
    state: closest.state,
    country: 'India',
    lat: Number(lat.toFixed(4)),
    lon: Number(lon.toFixed(4)),
