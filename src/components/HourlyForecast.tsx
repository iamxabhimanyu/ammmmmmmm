          ...
          {hourly.slice(0, 24).map((item, idx) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-between min-w-[64px] sm:min-w-[70px] py-3 px-2 rounded-2xl transition-all duration-150 shrink-0 select-none touch-manipulation active:scale-95 ${
                  isSelected
                ...
                aria-label={`Forecast for ${item.time}: ${displayTemp(item.temperature)} degrees, ${item.conditionText}`}
              >
                {/* Time Label */}
                <span
                  className={`text-xs font-medium tracking-tight mb-2 ${
                    isSelected ? 'text-slate-200' : isNow ? 'text-sky-700 font-semibold' : 'text-slate-500'
                  }`}
                >
                  {item.time}
                </span>
                ...
                <div className="my-1">
                  <WeatherIcon
                    ...
                    isDay={item.time.includes('AM') ? !item.time.startsWith('12') && !item.time.startsWith('1') && !item.time.startsWith('2') && !item.time.startsWith('3') && !item.time.startsWith('4') && !item.time.startsWith('5') : !item.time.startsWith('7') && !item.time.startsWith('8') && !item.time.startsWith('9') && !item.time.startsWith('10') && !item.time.startsWith('11')}
                    className={`w-6 h-6 ${isSelected ? 'text-amber-300' : ''}`}
                  />
                </div>

                {/* Temperature */}
                <span className={`text-base font-semibold mt-2 tracking-tight ${isSelected ? 'text-white' : 'text-slate-900'}`}>
