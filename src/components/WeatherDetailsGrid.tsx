  ...
  return (
    <section className="w-full max-w-2xl mx-auto px-4 my-2">
      <div className="flex items-center justify-between mb-2.5 px-1">
        <h3 className="text-sm font-semibold text-slate-800 tracking-tight">Weather Details</h3>
        <span className="text-xs text-slate-400 font-medium">Live Meteorological Data</span>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 gap-3">
        ...
        <div className="col-span-2 sm:col-span-1 bg-white/85 backdrop-blur-md rounded-3xl p-4 border border-black/[0.05] shadow-xs flex flex-col justify-between">
          ...
          <div className="my-1 flex items-baseline gap-2">
            ...
            <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${airQuality.bgColor}`}>
              {airQuality.category}
            </span>
          </div>
          ...
          <div className="flex items-center gap-3 pt-2 mt-2 border-t border-slate-100 text-[11px] text-slate-500">
            <span>PM2.5: <strong className="text-slate-700">{airQuality.pm25} µg/m³</strong></span>
            <span>PM10: <strong className="text-slate-700">{airQuality.pm10} µg/m³</strong></span>
          </div>
        </div>
        ...
        <div className="bg-white/85 backdrop-blur-md rounded-3xl p-3.5 sm:p-4 border border-black/[0.05] shadow-xs flex flex-col justify-between min-w-0">
          <div className="flex items-center justify-between text-xs text-slate-500 font-medium mb-2 gap-1 min-w-0">
            ...
            <span className="text-[11px] sm:text-xs font-semibold text-slate-700 flex items-center gap-1 truncate">
              ...
              <span className="truncate">{getWindDirectionLabel(weather.windDirection)} ({weather.windDirection}°)</span>
            </span>
          </div>
          ...
          <p className="text-xs text-slate-600 mt-1 truncate">
            Gusts up to <strong className="text-slate-800">{weather.windGust} km/h</strong>
          </p>
        </div>
        ...
        <div className="bg-white/85 backdrop-blur-md rounded-3xl p-4 border border-black/[0.05] shadow-xs flex flex-col justify-between">
          <div className="flex items-center justify-between text-xs text-slate-500 font-medium mb-2">
            ...
            <span className="text-xs font-medium text-slate-500">
              {weather.humidity > 65 ? 'Humid' : weather.humidity < 35 ? 'Dry' : 'Comfortable'}
            </span>
          </div>
          ...
          <p className="text-xs text-slate-600 mt-1">
            The dew point is <strong className="text-slate-800">{weather.dewPoint}°{temperatureUnit}</strong>
          </p>
        </div>
        ...
        <div className="bg-white/85 backdrop-blur-md rounded-3xl p-3.5 sm:p-4 border border-black/[0.05] shadow-xs flex flex-col justify-between min-w-0">
          <div className="flex items-center justify-between text-xs text-slate-500 font-medium mb-2">
            ...
            <span className="text-xs font-semibold text-slate-700">
              {weather.visibility >= 10 ? 'Clear' : weather.visibility >= 4 ? 'Moderate' : 'Foggy'}
            </span>
          </div>
      ...
      <div className="mt-3 text-center">
        <button
          ...
          className="inline-flex items-center gap-1.5 px-5 py-2 min-h-[44px] rounded-full bg-slate-900/5 hover:bg-slate-900/10 active:bg-slate-900/15 text-xs font-medium text-slate-700 transition-all focus:outline-none touch-manipulation active:scale-95"
        >
      ...
      {showMore && (
        <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-3 animate-in fade-in duration-200">
          <div className="bg-white/80 rounded-2xl p-3 border border-black/[0.04] text-xs">
            ...
            <span className="text-lg font-semibold text-slate-800 mt-0.5 block">{weather.pressure} hPa</span>
            <span className="text-[10px] text-slate-500">Normal barometric gradient</span>
          </div>

          <div className="bg-white/80 rounded-2xl p-3 border border-black/[0.04] text-xs">
            <span className="text-slate-400 font-medium block">Cloud Cover</span>
            <span className="text-lg font-semibold text-slate-800 mt-0.5 block">{weather.cloudCover}%</span>
            <span className="text-[10px] text-slate-500">Atmospheric sky density</span>
          </div>
          ...
          <div className="bg-white/80 rounded-2xl p-3 border border-black/[0.04] text-xs col-span-2 sm:col-span-1">
            ...
            <span className="text-lg font-semibold text-slate-800 mt-0.5 block">{weather.precipitation24h} mm</span>
            <span className="text-[10px] text-slate-500">Cumulative rainfall recorded</span>
          </div>
        </div>
      )}
    </section>
