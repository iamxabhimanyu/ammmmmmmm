...
export const WeatherChart: React.FC<WeatherChartProps> = ({
  ...
  const temps = slice.map((h) => displayTemp(h.temperature));
  ...
  const points = slice.map((item, idx) => {
    const x = paddingX + (idx / (slice.length - 1)) * (width - paddingX * 2);
          ...
          <div className="flex flex-wrap items-center gap-1.5 xs:gap-2">
            ...
            {activePoint && (
              <span className="text-[11px] xs:text-xs font-semibold px-2 py-0.5 rounded-full bg-slate-900/5 text-slate-700">
                {activePoint.item.time}: {activePoint.tempVal}°{temperatureUnit} ({activePoint.item.precipitationProb}% Rain)
              </span>
            )}
          </div>
          ...
          >
            ...
            {activeTab === 'temp' ? (
              ...
              <>
                ...
                {points.map((pt, idx) => {
                  const prob = pt.item.precipitationProb;
