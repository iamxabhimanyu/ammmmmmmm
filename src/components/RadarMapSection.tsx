...
export type MapLayerType = 'radar' | 'satellite' | 'temp' | 'wind' | 'risk' | 'stations';
export type BaseMapStyle = 'voyager' | 'dark' | 'osm';
...
export const RadarMapSection: React.FC<RadarMapSectionProps> = ({
  ...
  // Update base map tile URL if baseMapStyle changes
  useEffect(() => {
  ...
  const handleMapTap = useCallback(async (lat: number, lon: number) => {
    ...
    setTappedPoint({
      location: {
        id: `tap-${lat.toFixed(3)}-${lon.toFixed(3)}`,
  ...
  useEffect(() => {
    ...
    // Layer 3: TEMPERATURE LAYER (Regional thermal badges across India)
    else if (activeLayer === 'temp' && overlayGroupRef.current) {
      REGIONAL_OBSERVATION_STATIONS.forEach((st) => {
        const tempVal = temperatureUnit === 'F' ? Math.round((st.temp * 9) / 5 + 32) : st.temp;
        const tempColor =
          st.temp < 20
        ...
        const tempIcon = L.divIcon({
          className: 'temp-station-badge',
          html: `
            <div class="flex items-center gap-1 px-2 py-0.5 rounded-full shadow-md text-xs font-bold border ${tempColor} backdrop-blur-xs select-none cursor-pointer transform hover:scale-110 transition-transform">
              <span>${tempVal}°</span>
              <span class="text-[10px] font-normal opacity-90 hidden xs:inline">${st.name}</span>
            </div>
          `,
          iconSize: [60, 24],
          iconAnchor: [30, 12],
        ...
        const marker = L.marker([st.lat, st.lon], { icon: tempIcon });
        marker.on('click', () => handleMapTap(st.lat, st.lon));
        overlayGroupRef.current?.addLayer(marker);
      });
    }

    // Layer 4: WIND FLOW VECTOR LAYER
    else if (activeLayer === 'wind' && overlayGroupRef.current) {
      REGIONAL_OBSERVATION_STATIONS.forEach((st) => {
        const windIcon = L.divIcon({
          className: 'wind-station-badge',
          html: `
            <div class="flex items-center gap-1 px-2 py-1 rounded-full shadow-md text-[11px] font-semibold bg-slate-900/85 text-sky-200 border border-sky-400/40 backdrop-blur-md select-none cursor-pointer transform hover:scale-110 transition-transform">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 text-sky-300" style="transform: rotate(${st.windDirection}deg);" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="19" x2="12" y2="5"></line><polyline points="5 12 12 5 19 12"></polyline></svg>
              <span>${st.windSpeed} km/h</span>
            </div>
          `,
          iconSize: [80, 26],
          iconAnchor: [40, 13],
        ...
        const marker = L.marker([st.lat, st.lon], { icon: windIcon });
        marker.on('click', () => handleMapTap(st.lat, st.lon));
        overlayGroupRef.current?.addLayer(marker);
      });
    }
        ...
        circle.bindTooltip(
          `
            <div class="p-1 text-xs">
              ...
              <span class="text-rose-700 font-semibold text-[10px]">${zone.category} • ${zone.severity} Alert</span>
  ...
  }, [activeLayer, currentFrameIndex, rainViewerData, temperatureUnit, handleMapTap]);
        ...
        <div className="relative w-full h-[52vh] sm:h-[62vh] min-h-[390px] rounded-2xl overflow-hidden border border-slate-200/80 shadow-inner">
          ...
          <div className="absolute top-3 right-3 z-10 flex flex-col gap-1.5 bg-white/90 backdrop-blur-md rounded-2xl p-1 shadow-md border border-slate-200/70">
            ...
            <button
              ...
              onClick={() => {
                setBaseMapStyle((prev) =>
                  prev === 'voyager' ? 'dark' : prev === 'dark' ? 'osm' : 'voyager'
                );
              }}
              className="w-10 h-10 flex items-center justify-center rounded-xl hover:bg-slate-100 active:bg-slate-200 text-slate-700 active:scale-95 transition-all touch-manipulation text-[10px] font-bold uppercase"
              title={`Basemap style: ${baseMapStyle}`}
          ...
          {(activeLayer === 'radar' || activeLayer === 'satellite') && (
            <div className="absolute bottom-3 left-3 z-10 bg-slate-900/85 backdrop-blur-md text-white text-[10px] sm:text-[11px] font-medium px-3 py-1 rounded-full shadow-sm flex items-center gap-1.5">
              <span
              ...
              <>
                ...
                <div className="grid grid-cols-3 gap-2 mt-2.5 pt-2 border-t border-sky-100/80 text-center">
                  <div className="bg-white/80 rounded-xl p-1.5 border border-black/[0.04]">
                    <span className="text-[10px] text-slate-500 block">Rain Likelihood</span>
                    <span className="text-xs font-bold text-sky-700">
                      {tappedPoint.weather.cloudCover > 50 ? '35%' : '10%'}
                    </span>
                  </div>
                  <div className="bg-white/80 rounded-xl p-1.5 border border-black/[0.04]">
